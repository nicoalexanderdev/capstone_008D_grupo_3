...
export default function SentidoScreen() {
  ...
  const {
    ...
    onFinalText: (finalText) => {
      ...
      if (match) {
        // Coincidió → navegamos
        handleSentidoPressVoice(match);
      } else {
        // NO coincidió → mensaje de error por voz
        //Alert.alert("No se reconoció el sentido");
        Speech.speak(
          "No se reconoció el sentido. Por favor, repite el nombre del sentido destino.",
          // {
          //   language: "es-CL",
          //   onDone: () => {
          //     const names = cached.map((s) => s.estacion.name).join(", ");
          //     speakThenListen(
          //       `Sentidos disponibles: ${names}. Di el sentido que deseas seleccionar.`
          //     );
          //   },
          // }
        );
