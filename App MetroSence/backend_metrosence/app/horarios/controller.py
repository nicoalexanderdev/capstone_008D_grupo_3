...
@router.get("/estacion/{estacion_id}/formateado", response_model=model.HorarioFormateado)
def read_horario_formateado(estacion_id: int, db: Session = Depends(get_db)):
    db_horario = service.get_horario_formateado(db, estacion_id=estacion_id)
    if db_horario is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Horario no encontrado")
    return db_horario
...
@router.put("/estacion/{estacion_id}", response_model=model.Horario)
def update_horario(estacion_id: int, horario: model.HorarioUpdate, db: Session = Depends(get_db), current_admin: str = Depends(get_current_admin)):
    db_horario = service.update_horario(db, estacion_id=estacion_id, horario=horario)
    if db_horario is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Horario no encontrado")
    return db_horario

@router.delete("/estacion/{estacion_id}", response_model=model.Horario)
def delete_horario(estacion_id: int, db: Session = Depends(get_db), current_admin: str = Depends(get_current_admin)):
    db_horario = service.delete_horario(db, estacion_id=estacion_id)
    if db_horario is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Horario no encontrado")
    return db_horario